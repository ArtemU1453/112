...
| CAP-SEARCH-001 | Универсальный поиск (полнотекст/атрибуты/семантика) | ADR-028 | Self-hosted поисковый движок (абстрагирован; семантика через `CAP-AI-001`) | Approved | Stage 13 |
...
| CAP-KNOW-001 | Национальная платформа знаний и поддержки решений | ADR-028 | Self-hosted реестр знаний/SOP/правил/справочников | Approved | Stage 13 |

## Возможности со статусом Proposed

`CAP-DOC-001` (генерация/экспорт документов) — требования и абстракции (порты) зафиксированы на
Stage 4, конкретный движок не утверждён. Выбор технологии — через RFC + новый ADR с обновлением данной
таблицы (TAP «Изменение технологии»); до утверждения документы и код ссылаются на возможность и порт,
а не на продукт.

Ранее предложенные `CAP-SECRETS-001`, `CAP-STORAGE-001`, `CAP-TRACE-001` переведены в Approved на
Stage 3 (ADR-011/013/012, RFC-0002). `CAP-SEARCH-001` (универсальный поиск) переведена в Approved на
Stage 13 (ADR-028, RFC-0010) — self-hosted движок абстрагирован, семантика через `CAP-AI-001`; частично
закрывает TD-2026-001.
